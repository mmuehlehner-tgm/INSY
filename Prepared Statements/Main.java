...
public class Main {
	...
	public static void main(String[] args) throws Exception {
		...
		// Wenn die Verbindung funktioniert (also nicht null zurück liefert)
		// wird die Conncetion gespeichert
		
		if (dbcon.connect() != null) {
			c = dbcon.connect();
		} else { // Wenn nicht, wird versucht sich durch die Arguments mit der
					// Datenbank zu verbinden
			try {
				dbcon = new DBConnection(argsstring);
				c = dbcon.connect();
			} catch (Exception ex) { // Wenn das auch nicht geht, wird der
										// Benutzer durch eine Ausgabe
										// informiert
